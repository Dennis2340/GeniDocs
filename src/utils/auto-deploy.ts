  ...
  if (IS_PRODUCTION) {
    ...
    try {
      const child = spawn(NPM_CMD, ["start"], {
        ...
        shell: isWindows // Set shell to true on Windows systems to prevent EINVAL errors
      });
      ...
      log(`Error starting Docusaurus server: ${error}`);
      log("Attempting fallback method...");
      
      // Fallback to using exec instead of spawn
      try {
        const command = isWindows ? 
          `start cmd.exe /K "cd ${DOCS_DIR} && npm start"` : 
          `cd ${DOCS_DIR} && npm start &`;
          
        await execAsync(command);
        log("Docusaurus server started using fallback method");
        log("You can access the documentation at: http://localhost:3001");
      } catch (fallbackError) {
        log(`Fallback method also failed: ${fallbackError}`);
        throw new Error(`Failed to start Docusaurus server: ${error}`);
      }
    }
  }
}
